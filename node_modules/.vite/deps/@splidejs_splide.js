//#region node_modules/@splidejs/splide/dist/js/splide.esm.js
...
function Elements(Splide2, Components2, options) {
	...
	function find(selector) {
		...
		return elm && closest(elm, ".splide") === root ? elm : void 0;
	}
...
function Slide$1(Splide2, index, slideIndex, slide) {
	...
	function listen() {
		...
		bind(slide, "keydown", apply(emit, "sk", self));
		on([
			EVENT_MOVED,
			"sh",
			EVENT_SCROLLED
	...
	function updateActivity() {
		var active = isActive();
		if (active !== hasClass(slide, "is-active")) {
	...
	function updateVisibility() {
		...
		if (visible !== hasClass(slide, "is-visible")) {
...
function Clones(Splide2, Components2, options) {
	...
	function computeCloneCount() {
		...
		if (!Splide2.is("loop")) clones2 = 0;
			...
			clones2 = fixedSize && ceil(rect(Elements.track)[resolve("width")] / fixedSize) || options[resolve("autoWidth")] && Splide2.length || options.perPage * MULTIPLIER;
		}
		return clones2;
	}
	return {
...
function Move(Splide2, Components2, options) {
	...
	function loop(position) {
		if (Splide2.is("loop")) {
			...
			if (index < 0 || exceededMax) position = shift(position, exceededMax);
		}
		return position;
	}
...
function Controller(Splide2, Components2, options) {
	...
	function computeDestIndex(dest, from, snapPage) {
		if (isEnough() || hasFocus()) {
			...
			if (dest < 0 || dest > endIndex) if (!perMove && (between(0, dest, from, true) || between(endIndex, from, dest, true))) dest = toIndex(toPage(dest));
			else if (isLoop) dest = snapPage ? dest < 0 ? -(slideCount % perPage || perPage) : slideCount : dest;
...
function Arrows(Splide2, Components2, options) {
	...
	function createArrow(prev2) {
		return parseHtml("<button class=\"" + classes.arrow + " " + (prev2 ? classes.prev : classes.next) + "\" type=\"button\"><svg xmlns=\"" + XML_NAME_SPACE + "\" viewBox=\"0 0 " + SIZE + " " + SIZE + "\" width=\"" + SIZE + "\" height=\"" + SIZE + "\" focusable=\"false\"><path d=\"" + (options.arrowPath || PATH) + "\" />");
	}
	function update() {
		if (prev && next) {
...
function Scroll(Splide2, Components2, options) {
	...
	function update(from, to, noConstrain, rate) {
		var position = getPosition();
		var diff = (from + (to - from) * easing(rate) - position) * friction;
...
function Drag(Splide2, Components2, options) {
	...
	function isDraggable(target2) {
		...
		return !matches(target2, ".splide__pagination__page, .splide__arrow") && (!noDrag || !matches(target2, noDrag));
	}
...
function Sync(Splide2, Components2, options) {
	...
	return {
		setup: apply(Components2.Media.set, { slideFocus: isUndefined(slideFocus) ? isNavigation : slideFocus }, true),
		mount,
		destroy,
		remount
	};
...
function Slide(Splide2, Components2, options) {
	...
	function start(index, done) {
		...
		if (abs(destination - position) >= 1 && speed >= 1) if (options.useScroll) Scroll.scroll(destination, speed, false, done);
		else {
...
var Splide = /* @__PURE__ */ function() {
	...
	_proto.mount = function mount(Extensions, Transition) {
		...
		forOwn(assign({}, ComponentConstructors, this._E, { Transition: this._T }), function(Component, key) {
...
var SplideRenderer = /* @__PURE__ */ function() {
	...
	SplideRenderer.clean = function clean(splide) {
		var on = EventInterface(splide).on;
	...
	_proto3.buildClasses = function buildClasses() {
		...
		return [
			...
			options.drag && "splide--draggable",
			options.isNavigation && "splide--nav",
			CLASS_ACTIVE,
	...
	_proto3.generateClones = function generateClones(contents) {
		...
		push(slides.slice(-count).reverse(), slides.slice(0, count)).forEach(function(content, index) {
			var clone = assign({}, content, { attrs: assign({}, content.attrs, { class: content.attrs.class + " " + classes.clone }) });
	...
	_proto3.getCloneCount = function getCloneCount() {
		if (this.isLoop()) {
			...
			return max.apply(void 0, this.breakpoints.map(function(_ref6) {
				return _ref6[1].perPage;
			})) * ((options.flickMaxPages || 1) + 1);
		}
		return 0;
	};
